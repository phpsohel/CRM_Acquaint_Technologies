...
class ProductController extends Controller
{
    ...
    public function product_detail($id)
    {
        ...
        $detail = Product::orderBy('id', 'DESC')
                            ->where('id', '=', $id)
                            ->first();
        $lims_product_data = Product::where('id', $id)->select('image','product_details')->first();
        ...
        $transfer = Transfer::orderBy('transfers.id', 'DESC')
                        ->join('product_transfer', 'product_transfer.transfer_id', '=', 'transfers.id')
                        ->where('product_transfer.product_id', '=', $id)
                        ->get();

        $adjustment = Adjustment::orderBy('adjustments.id', 'DESC')
                        ->join('warehouses', 'warehouses.id', '=', 'adjustments.warehouse_id')
                        ->join('product_adjustments', 'product_adjustments.adjustment_id', '=', 'adjustments.id')
                        ->where('product_adjustments.product_id', '=', $id)
                        ->select('adjustments.created_at', 'adjustments.reference_no', 'warehouses.name', 'adjustments.note')
                        ->get();

        $return = Returns::orderBy('returns.id', 'DESC')
                        ->join('product_returns', 'product_returns.return_id', '=', 'returns.id')
                        ->where('product_returns.product_id', '=', $id)
                        ->get();

        $return_purchase = ReturnPurchase::join('purchase_product_return', 'purchase_product_return.return_id', '=', 'return_purchases.id')
                        ->where('purchase_product_return.product_id', '=', $id)
                        ->get();
        
        // return $purchase; exit();
        return view('product.detail', compact('detail', 'lims_product_data', 'sale', 'purchase', 'transfer', 'adjustment', 'tax', 'return', 'return_purchase'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $lims_product_data = Product::create($data);
             ...
             $pro_warehouse->product_id = $lims_product_data->id;
             $pro_warehouse->warehouse_id = 1;
             $pro_warehouse->qty = $lims_product_data->qty;
             $pro_warehouse->price = $lims_product_data->price;
             $pro_warehouse->save();
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            // return $lims_product_data->product_details; exit();
