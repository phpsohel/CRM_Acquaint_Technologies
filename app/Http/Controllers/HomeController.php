...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $recent_lead = Lead::orderBy('id', 'desc')->take(5)->get();
            ...
            $recent_project = Project::orderBy('id', 'desc')->take(5)->get();
            $recent_ticket = Ticket::orderBy('id', 'desc')->take(5)->get();
        }
        else {
        ...
        while($start < $end)
        {
           ...
           // monthly
            ...
            //$start_m = strtotime(date("Y") .'-01-01');
          //  $end_m = strtotime(date("Y") .'-01-31');


            //$start_jan = date("Y").'-'.date('m', $start_m).'-'.'01';
           // $end_jun = date("Y").'-'.date('m', $end_m).'-'.'31';



            $monthly_lead = Lead::whereDate('created_at', '>=' , $monthStart)->whereDate('created_at', '<=' , $monthEnd)->sum('id');
          //  $monthly_lead = Lead::whereDate('created_at', '>=' , $monthStart)->whereDate('created_at', '<=' , $monthEnd)->count();
        ...
        {
            $total_leads = Lead::where('status',true)->get()->count();
        }else{
            $total_leads = Lead::where('status',true)->where('employee_id',$authuser->id)->get()->count();
        }
       

        $total_quotation = Quotation::get()->count();
        $total_sale = Sale::get()->count();
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $total_reminder = Remainder::where('status',true)->where('user_id', Auth::id())->get()->count();
            $total_reminder_complete = Remainder::where('stage',1)->where('status',true)->where('user_id', Auth::id())->get()->count();
            $total_meeting = Lead::where('lead_status_id',7)->where('status',true)->where('user_id', Auth::id())->get()->count();
        }
        else {
            $total_reminder = Remainder::where('status',true)->get()->count();
            $total_reminder_complete = Remainder::where('stage',1)->where('status',true)->get()->count();
            $total_meeting = Lead::where('lead_status_id',7)->where('status',true)->get()->count();
        }
        ...
        //return $month;
        return view('index', compact('total_meeting','total_reminder_complete','total_reminder','recent_ticket','recent_project','recent_lead','monthly_lead','total_project','total_sale','total_quotation','total_leads','revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price'));
    }
    ...
    public function todays_reminder()
    {
        ...
        if($role->name == 'Admin')
        {
            $lims_reminder_data = \App\Remainder::whereDate('noti_datetime',$today)->where('status',true)->get();
        }else{
            $lims_reminder_data = \App\Remainder::whereDate('noti_datetime',$today)->where('user_id',Auth()->user()->id)->where('status',true)->get();
        }
       
        $lims_user_list = User::where('role_id','!=',5)->where('is_active', true)->get();
        $lims_lead_list = Lead::where('status', true)->get();

        return view('remainder.todays-reminder', compact('lims_lead_list','lims_user_list','lims_reminder_data'));
    }
}
