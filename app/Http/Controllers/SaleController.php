...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
...

            {
               ...
               // $nestedData['biller'] = 'N/A';
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            if(in_array("create_project", $request['all_permission'])){
                                    $nestedData['options'] .= '<li>
                                     <a href="'.route('project_create', $sale->id).'" class="btn btn-link"><i class="fa fa-plus"></i> '.trans('Create Project').'</a>
                                   </li>';
                            }

                            if(in_array("add_payment", $request['all_permission'])){
                                $nestedData['options'] .= '<li>
                ...
                if(in_array("view_payment", $request['all_permission'])){
                    $nestedData['options'] .= '<li>
          ...
          // $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' "'.$sale->biller->name.'"', ' "'.$sale->biller->company_name.'"', ' "'.$sale->biller->email.'"', ' "'.$sale->biller->phone_number.'"', ' "'.$sale->biller->address.'"', ' "'.$sale->biller->city.'"', ' "'.$sale->customer->name.'"', ' "'.$sale->customer->phone_number.'"', ' "'.$sale->customer->address.'"', ' "'.$sale->customer->city.'"', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$sale->total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$sale->order_tax.'"', ' "'.$sale->order_tax_rate.'"', ' "'.$sale->order_discount.'"', ' "'.$sale->shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.preg_replace('/\s+/S', " ", $sale->sale_note).'"', ' "'.preg_replace('/\s+/S', " ", $sale->staff_note).'"', ' "'.$sale->user->name.'"', ' "'.$sale->user->email.'"', ' "'.$sale->warehouse->name.'"', ' "'.$coupon_code.'"', ' "'.$sale->coupon_discount.'"]' );
           $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' "'.$sale->user->name.'"', ' "'.$sale->user->email.'"', ' "'.$sale->user->company_name.'"', ' "'.$sale->user->phone_number.'"',  ' "'.$sale->user->phone_number.'"',   ' "'.$sale->user->phone_number.'"',  ' "'.$sale->customer->name.'"', ' "'.$sale->customer->phone_number.'"', ' "'.$sale->customer->address.'"', ' "'.$sale->customer->city.'"', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$sale->total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$sale->order_tax.'"', ' "'.$sale->order_tax_rate.'"', ' "'.$sale->order_discount.'"', ' "'.$sale->shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.preg_replace('/\s+/S', " ", $sale->sale_note).'"', ' "'.preg_replace('/\s+/S', " ", $sale->staff_note).'"', ' "'.$sale->user->name.'"', ' "'.$sale->user->email.'"', ' "'.$sale->warehouse->name.'"', ' "'.$coupon_code.'"', ' "'.$sale->coupon_discount.'"]' );
    ...
    public function store(Request $request)
    {
       ...
       // return ($data);
        ...
        if(!empty($lead))
        {
            ...
            // $user->phone = $lead->phone_number;
            // $user->company_name = $lead->name;
            
            $customer = new Customer();
            $customer->lead_id = $lead->id ?? 'Na';
            $customer->name = $lead->name ?? 'Na';
            $customer->company_name = $lead->company ?? 'Na';
            $customer->email = $lead->email ?? 'Na';
            $customer->address = $lead->address ?? 'Na';
            $customer->phone_number = $lead->phone_number ?? '0';
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = $lims_product_data->alert_quantity;
        return $product;
