...
class QuotationController extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('quotes-add')){
            ...
            $lims_lead_list = Lead::where('status', true)->select('id','name','company')->get();
            $lims_supplier_list = Supplier::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();

            return view('quotation.create', compact('lims_lead_list','lims_biller_list', 'lims_warehouse_list', 'lims_customer_list', 'lims_supplier_list', 'lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->quotation_status == 2 && $request->stage == 2 ){
            $lims_lead_data = Lead::find($data['lead_id']);
            $customer = new Customer();
            $customer->lead_id = $lims_lead_data->id;
            $customer->name = $lims_lead_data->name;
            $customer->company_name = $lims_lead_data->company;
            $customer->web = $lims_lead_data->web;
            $customer->email = $lims_lead_data->email;
            $customer->phone_number = $lims_lead_data->phone_number;
            $customer->address = $lims_lead_data->address;
            $customer->is_active = $lims_lead_data->status;
            $customer->save();
        }
        ...
        if($lims_quotation_data->quotation_status == 2){
            ...
            $lims_customer_data = Lead::find($data['lead_id']);
    ...
    {
        $lims_customer_data = Lead::where('id',$id)->first();
...
//  ----old------
    ...
    public function getProduct($id)
    {
      ...
      //  $product_data = [$product_code, $product_name, $product_qty, $product_name, $product_name, $product_name, $product_name, $product_price];
        return $product_data;
    }
...
//        $product_price = [];
//        $product_data = [];
//        // $lims_products_data = Product::where('products.is_active', true)->get();
//        $lims_products_data = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
//            ->where([
//                ['products.is_active', true],
//                ['product_warehouse.warehouse_id', $id],
//            ])->whereNull('product_warehouse.variant_id')->select('product_warehouse.*')->get();
//        foreach ($lims_products_data as $product)
//        {
//            $lims_product_data = Product::find($product->product_id);
//            $product_code[] = $lims_product_data->code;
//            $product_name[] = $lims_product_data->name;
//            $product_type[] = $lims_product_data->type;
//            $product_id[]   = $lims_product_data->id;
...
//
//        //retrieve product data of digital and combo
//        $lims_productss_data = Product::whereNotIn('type', ['standard'])->where('is_active', true)->get();
//        foreach ($lims_productss_data as $product)
//        {
//            $lims_product_data = Product::find($product->id);
//            $product_code[] =  $lims_product_data->code;
//            $product_name[] = $lims_product_data->name;
//            $product_type[] = $lims_product_data->type;
//            $product_id[] = $lims_product_data->id;
...
//        }
//        $product_data = [$product_code, $product_name, $product_type, $product_id,$product_qty, $product_list, $product_price];
//        return $product_data;
//    }
...
//
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('quotes-edit')){
            ...
            $lims_product_quotation_data = ProductQuotation::where('quotation_id', $id)->get();
            return view('quotation.edit',compact('lims_lead_list','lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_quotation_data','lims_product_quotation_data', 'lims_supplier_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->quotation_status == 2 && $stage == 2 ){
            $lims_lead_data = Lead::find($data['lead_id']);
            $customer = new Customer();
            $customer->lead_id = $lims_lead_data->id;
            $customer->name = $lims_lead_data->name;
    ...
    {
        $lims_quotation_data = Quotation::find($id);
        $lead = Lead::where('id',$lims_quotation_data->lead_id)->select('id','name','company')->first();

       // return  $lead;
       // $lims_customer_list = Customer::where('lead_id', $lims_quotation_data->lead_id)->where('is_active', true)->get();
       // return $lims_customer_list;
        ...
        return view('quotation.create_sale',compact('lead', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_quotation_data','lims_product_quotation_data', 'lims_pos_setting_data'));
    }
