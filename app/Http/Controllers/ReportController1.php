...
class ReportController extends Controller
{
    ...
    //exporting pdf daily sale
    public function daily_sale_pdf($year, $month)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('daily-sale')){
            $start = 1;
            ...
            $warehouse_id = 0;
            $date =  date('Y-m-d');
            $setting = ReportSetting::latest()->first();

            $pdf = PDF::loadview('pdf.report_daily_sale', compact('setting','date','total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
     ...
     //exporting print daily sale
    public function daily_sale_print($year, $month)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('daily-sale')){
            $start = 1;
            ...
            return view('print.daily_sale_print', compact('setting','date','total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
    ...
    //pdf daily purchase 
    public function daily_purchase_pdf($year, $month)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('daily-purchase')){
            $start = 1;
            ...
            $date =  date('Y-m-d');
            $setting = ReportSetting::latest()->first();
            $pdf = PDF::loadview('pdf.report_daily_purchase', compact('setting','date','total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
    ...
    public function daily_purchase_print($year, $month)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('daily-purchase')){
            $start = 1;
            ...
            return view('print.daily_purchase_print', compact('setting','date','total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
    ...
    //monthly sale pdf 
    public function monthly_sale_pdf($year)
    {
       $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('monthly-sale')){
            $start = strtotime($year .'-01-01');
            ...
            $pdf = PDF::loadview('pdf.report_monthly_sale', compact('setting','date','year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'total', 'lims_warehouse_list', 'warehouse_id'));
    ...
    //monthly sale print
    public function monthly_sale_print($year)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('monthly-sale')){
            $start = strtotime($year .'-01-01');
            ...
            return view('print.monthly_sale_print', compact('setting','date','year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'total', 'lims_warehouse_list', 'warehouse_id'));
    ...
    //monthly purchase pdf 
    public function monthly_purchase_pdf($year)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('monthly-purchase')){
            $start = strtotime($year .'-01-01');
            ...
            $setting = ReportSetting::latest()->first();
            $date =  date('Y-m-d');
            // return $setting->site_logo; exit();
            // return view('report.monthly_purchase', compact('year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'lims_warehouse_list', 'warehouse_id'));




            $pdf = PDF::loadview('pdf.report_monthly_purchase', compact('date','setting','year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'lims_warehouse_list', 'warehouse_id'));
    ...
    //monthly purchase print
    public function monthly_purchase_print($year)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('monthly-purchase')){
            $start = strtotime($year .'-01-01');
            ...
            // return view('report.monthly_purchase', compact('year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'lims_warehouse_list', 'warehouse_id'));
             ...
             return view('print.monthly_purchase_print', compact('year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'lims_warehouse_list', 'warehouse_id', 'setting', 'date'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    ...
    //monthy purchase by warehouse pdf
    public function pdf_monthly_purchase_warehouse()
    {
        # code...
    }
    ...
    //EXPORTING PDF of report summary 
    public function export_pdf($start_date, $end_date)
    {
        ...
        $pdf = PDF::loadview('pdf.report_summary', compact('setting','date','purchase', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    ...
    //EXPORTING Print of report summary 
    public function summary_print($start_date, $end_date)
    {
        ...
        return view('print.summary_report_print', compact('setting','date','purchase', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    }
    ...
    public function leadEmployeeReport(Request $request)
    {
        $data = $request->all();
        ...
        $lead_cat_id = $data['lead_cat_id'];
        if ($lead_cat_id == 0 &&  $employee_id == 0 ){
    ...
    //  dd(count($lims_lead_data));
            ...
            $lims_lead_data = Lead::where('lead_category_id',$lead_cat_id)->where('employee_id',$employee_id)->whereDate('date', '>=' , $start_date)->whereDate('date', '<=' , $end_date)->where('status', true)->get();
        }
        $lims_employee_list = Employee::where('is_active', true)->get();
        $lims_lead_cat_list = LeadCategory::where('status', true)->get();
        return view('report.lead_employee_report',compact('lims_lead_cat_list','lead_cat_id','lims_lead_data' ,'start_date', 'end_date', 'lims_employee_list','employee_id'));
    }
    ...
    public function leadStatusReport(Request $request)
{
    ...
    public function leadReminderReport(Request $request)
    {
        ...
        $employee_id = $data['employee_id'];
        $stage = $data['stage'];
        if ($stage == 2 &&  $employee_id == 0 ){
            $lims_reminder_data = Remainder::whereDate('noti_datetime', '>=' , $start_date)->whereDate('noti_datetime', '<=' , $end_date)->where('status', true)->get();
        }elseif($stage == 2 ){
            $lims_reminder_data = Remainder::where('employee_id',$employee_id)->whereDate('noti_datetime', '>=' , $start_date)->whereDate('noti_datetime', '<=' , $end_date)->where('status', true)->get();
        }elseif($employee_id == 0){
            $lims_reminder_data = Remainder::where('stage',$stage)->whereDate('noti_datetime', '>=' , $start_date)->whereDate('noti_datetime', '<=' , $end_date)->where('status', true)->get();

        }else{
            $lims_reminder_data = Remainder::where('stage',$stage)->where('employee_id',$employee_id)->whereDate('noti_datetime', '>=' , $start_date)->whereDate('noti_datetime', '<=' , $end_date)->where('status', true)->get();
        }
        $lims_employee_list = Employee::where('is_active', true)->get();
//        $lims_status_list = LeadStatus::where('status', true)->get();
        return view('report.lead_reminder_report',compact('lims_employee_list','stage','lims_reminder_data' ,'start_date', 'end_date','employee_id'));
    }
    ...
    public function customers_message()
    {
        $contact = Contact::orderBy('id', 'DESC')->get();
        return view('report.customers_message', compact('contact'));
    }

    public function view_newsletter()
    {
        $newsletter = Newsletter::orderBy('id', 'DESC')->get();
        return view('report.view_newsletter', compact('newsletter'));
    }

    public function expire_report()
    {
        $purchase = Purchase::orderBy('purchases.id', 'DESC')
                                ->join('product_purchases', 'product_purchases.purchase_id', '=', 'purchases.id')
                                ->join('products', 'products.id', 'product_purchases.product_id')
                                ->select('purchases.created_at', 'purchases.reference_no', 'product_purchases.expire_date', 'product_purchases.qty', 'products.name')
        ...
        return view('report.expire_report', compact('purchase', 'start_date', 'end_date'));
    }

    public function expire_date_filter(Request $request)
    {
        $purchase = Purchase::orderBy('purchases.id', 'DESC')
                                ->join('product_purchases', 'product_purchases.purchase_id', '=', 'purchases.id')
                                ->join('products', 'products.id', 'product_purchases.product_id')
                                ->select('purchases.created_at', 'purchases.reference_no', 'product_purchases.expire_date', 'product_purchases.qty', 'products.name')
                                ->where('products.is_active', 1)
                                ->where('product_purchases.expire_date', '>=', $request->start_date)
                                ->where('product_purchases.expire_date', '<=', $request->end_date)
                                ->get();
        $start_date = $request->start_date;
        $end_date = $request->end_date;
